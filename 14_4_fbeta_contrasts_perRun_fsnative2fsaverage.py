            ...
            for hemi in ["L", "R"]:
                ...
                for contrast_name, (pos_cond, neg_cond) in contrasts.items():
                    for map in data_type:  
                        ...
                        source_fsnative_file = op.join(glm_dir, f"{participant}_task-{design}_run-{run+1}_hemi-{hemi}_space-fsnative_desc-{contrast_name}_{map}.mgz")
                        if not os.path.exists(source_fsnative_file): 
                            continue
                        out_fsaverage_file = op.join(surf_map_dir, f"{participant}_task-{design}_run-{run+1}_hemi-{hemi}_space-fsaverage_desc-{contrast_name}_{map}.mgz")
