...
participants.remove('sub-NSxLxYKx1964')
...
for hemi in {"L", "R"}:
    ...
    for p, participant in enumerate(participants): 
        ...
        for t, tract in enumerate(tract_names):
            ...
            tdi_path = op.join(bids_path, 'analysis', 'tdi_maps', 'dipy_wmgmi_tdi_maps', participant, 'pyAFQ3_wb')
            os.makedirs(tdi_path, exist_ok=True)

            if "CallosumOccipital" in tract:
                tract_path = Path(os.path.join(afq_TR_path, participant, 'bundles', f"{participant}_ses-concat_acq-HCPdir99_desc-{tract}_tractography.trx"))
            elif any(substring in tract for substring in ["TR", "MTPTxSTS1", "MTLGNxPU", "MTFEF"]):
                tract_path = Path(os.path.join(afq_TR_path, participant, 'bundles', f"{participant}_ses-concat_acq-HCPdir99_desc-{hemi}{tract}_tractography.trx"))
            else:
                tract_path = Path(os.path.join(afq_TR_path, participant, 'bundles', f"{participant}_ses-concat_acq-HCPdir99_desc-{hemi_afq}{tract}_tractography.trx"))
            
            if tract_path.exists():
                if "CallosumOccipital" in tract:
                    tract_tdi_map = os.path.join(tdi_path, participant+'_ses-concat_desc-'+f"{tract}"+'_tdi_map.nii.gz')
                else:
                    tract_tdi_map = os.path.join(tdi_path, participant+'_ses-concat_desc-'+f"{hemi_afq}{tract}"+'_tdi_map.nii.gz')
                template = op.join(afq_TR_path, participant, participant+'_ses-concat_acq-HCPdir99_b0ref.nii.gz')
...
##--------------------------------------------
# Quantify overlap between thalamic radiations and julich based tracts using endpoint density maps
##--------------------------------------------
import os
import os.path as op
import pandas as pd
        ...
        for tr in TR_tract_names:
            for targ in targ_tract_names:
                if "CallosumOccipital" in tr:
                    TR_path = op.join(density_dir, participant, 'pyAFQ3_wb', participant+'_ses-concat_desc-' + f"{tr}" + '_tdi_map.nii.gz')
                else:
                    TR_path = op.join(density_dir, participant, 'pyAFQ3_wb', participant+'_ses-concat_desc-' + f"{hemi_afq}{tr}" + '_tdi_map.nii.gz')
                    
                targ_path = op.join(density_dir, participant, 'pyAFQ3_wb', participant+'_ses-concat_desc-' + f"{hemi_afq}{targ}" + '_tdi_map.nii.gz')
...
for ax, hemi in zip(axes, ["L", "R"]):
    ...
    pivot = (
        tmp.groupby(["TR_tract", "targ_tract"])["percent_TR"]
        .mean()
        .unstack()
    )
...
plt.savefig(op.join(saveDir, f"pyafq3_15rounds_tracts_overlap_percentAtlas.png"),
