...
def main(participants_file, tract_name, bids_path, pyAFQ_path):
    '''
        Ex usage: python 11_endpoint_density_mri_vol2surf.py --participants_file ./utils/study2_subjects_updated.txt --tract_name LeftMTxLGN --bids_path /Use
    ...
    for participant in participants_file:
        ...
        afq_path = op.join(pyAFQ_path, tract_name, participant)
        ...
        hemisphere = "L" if "Left" in tract_name else "R" if "Right" in tract_name else None
        ...
        for h, hemi in enumerate(hemisphere):
            ...
            density_path = op.join(out_path, f"{participant}_ses-concat_desc-{mask_name}_tdi_map.nii.gz")
        ...
        # -----------------------------
            ...
            # -----------------------------
            mt_density_path = op.join(analysis_path, "tdi_maps", "wmgmi_voxels", participant,f"{participant}_hemi-{hemi}_label-MTxWMGMI_density.nii.gz")
            ...
            # Output file name
            out_file = os.path.join(out_path, f"{participant}_hemi-{hemi}_space-fsnative_label-{mask_name}_desc-fsprojdensity0mm.mgh")
