...
hemis = {"Left", "Right"}
tract_name = "afq-RightMTxFEF"

wb_tract_file = op.join(afq_path, tract_name, participant, "tractography",
                        f"{participant}_ses-concat_acq-HCPdir99_tractography.trx")

trk = load_tractogram(wb_tract_file, t1w_img)
trk.to_rasmm()
trk_xfm = transform_streamlines(trk.streamlines, np.linalg.inv(t1w_img.affine))
tract_actor = lines_as_tubes(trk_xfm, 5, color=(0.2, 0.6, 1))
