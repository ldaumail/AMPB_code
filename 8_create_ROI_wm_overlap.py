...
def main(participants_file, bids_path):
    for participant in participants_file:
        ...
        for mask in ['MT', 'PT']:
            for hemi in ['L', 'R']:
                input_file2 = op.join(bids_path, 'analysis', 'functional_vol_roi', participant, participant+'_hemi-'+hemi+'_space-T1w_label-'+mask+'_mask_dilated.nii.gz')
                input_files = [input_file1, input_file2]
                output_file = op.join(bids_path, 'analysis', 'functional_vol_roi', participant, participant+'_hemi-'+hemi+'_space-T1w_label-'+mask+'xWM_mask.nii.gz')
