function run_GLMsingle(participant, glmMode)
...
paths.software = fullfile('/Users', 'ldaumail3', 'Documents', 'MATLAB');
addpath(genpath(fullfile('/Applications', 'freesurfer', '8.0.0-beta', 'matlab')));
addpath(genpath(fullfile(paths.software, 'GLMsingle'))); 
...
%% Perform GLMsingle and Save Beta Weights as Surface Files
...
for i = 4:length(conds) % for each task
    ...
    % determine functional data session
    clear opt; % clear previous
    opt.sessionindicator = str2double( ...
        regexprep(currBold, '.+_ses-(\d+)b?_.+', '$1'));
    
    % create data matrix and design matrix
    ...
    if ~isfile(fullfile(paths.glm, 'CONDITION_NAMES.mat')) 
    ...
    switch conds(i).task
        case 'ampb' % separate by runs and save
            %%% keep beta weights that were saved as percent signal change
            modelmd = squeeze(results.modelmd); % remove singleton dimensions

            %%% extract time index from each design matrix
            [~,indx] = cellfun(@(x) find(x), designSINGLE, ...
                'UniformOutput', false); 

            switch glmMode
                case 'surface'
                    %%% save each trial beta weight as a surface map (mgz)
                    for i2 = 1:length(currBold) % for each run
                        clear mri; mri.vol = shiftdim(modelmd(:,indx{i2}), -2);
                ...
                case 'volume'
                    %%% save each trial beta weight as a volume map (nii.gz)
                    for i2 = 1:length(currBold) % for each run
                        clear mri; mri.vol = modelmd(:,:,:,indx{i2});
                        mri.nframes = size(mri.vol, 4); % number of frames
                        [~,fname] = fileparts(currBold{i2}); % current run filename
                        saveName = regexprep(fname, '_bold.nii', '_beta.nii.gz');
                        if ~isfile(fullfile(paths.save, saveName))
                            MRIwrite(mri, fullfile(paths.save, saveName));
                            fprintf('Saved: %s\n', saveName);
                        end
        ...
        case {'mtlocal', 'ptlocal'} % collapse across runs and perform constrast
            ...
            modelmd = bsxfun(@times, modelmd, results.meanvol); % amplitude
            ...
            e = q .* var(cat(3, dataMat{:}), [], 2:3, 'omitnan'); % variance of residuals    
...
function [indx] = struct_contains(list, conds)
    indx = true(size(list)); flds = fieldnames(conds);
    for i = 1:length(flds)
        indx = indx & contains(list, conds.(flds{i})); 
    end
end
