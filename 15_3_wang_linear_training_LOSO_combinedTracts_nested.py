...
for h, hemi in enumerate(hemis):
    ...
    for test_idx in range(n_subj):
        ...
        trained_coefs[:, test_idx, h] = linreg.coef_.copy() #[0:n_tracts-1]
        # ridge.intercept_
...
plt.savefig(op.join(saveDir, "pearson_mean_linreg_loso_combined_tracts.png"), dpi=300, bbox_inches='tight')
plt.show()
...
saveDir = op.join(bids_path, 'analysis', 'plots')
os.makedirs(saveDir, exist_ok=True)
plt.savefig(op.join(saveDir, "betas_linreg_loso_combined_tracts.png"),
