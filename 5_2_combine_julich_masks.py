#This script was developped to create and resample Julich ROIs from MNI space to individual subject diffusion space (ACPC)
# For each participant
#Loic Daumail - 17/12/2025
import os
...
def main(participants_file):
    ...
    for participant in participants_file:
        ...
        paths_ACPC = op.join(paths_local, 'analysis','ROIs', 'julich_space-ACPC_rois', participant)
        ...
        # prob_roi_list = ['Area-Te-2.1_lh_MNI152.nii.gz', 'Area-STS1_lh_MNI152.nii.gz', 'Metathalamus-CGL_lh_MNI152.nii.gz', 'Area-8v1_lh_MNI152.nii.gz',
        #                  'Area-Te-2.1_rh_MNI152.nii.gz', 'Area-STS1_rh_MNI152.nii.gz', 'Metathalamus-CGL_rh_MNI152.nii.gz', 'Area-8v1_rh_MNI152.nii.gz']
        # mask_name = ['lhPT', 'lhSTS1', 'lhLGN', 'lhFEF',
        #             'rhPT', 'rhSTS1', 'rhLGN', 'rhFEF']
        
        #-------------------------------
        #1: First generate a LGN/PU mask
        #-------------------------------

        ##Create pulvinar mask
        for hemi in ['lh', 'rh']:
            #LGN
            prob_LGN_roi = 'Metathalamus-CGL_'+hemi+'_MNI152.nii.gz'
            ...
            transformed_mask_path = op.join(paths_ACPC, 'ses-concat', 'anat', participant+'_hemi-'+hemi+'_space-ACPC_desc-'+mask_name+'_mask.nii.gz')
            if os.path.exists(transformed_mask_path):
                print("File exists!")
            else:
                print("File does not exist. Creating it now")
                # load julich masks
                LGN_julich_mask = op.join(paths_julich, prob_LGN_roi)
                LGN_julich_mask_img = ants.image_read(LGN_julich_mask)
                ...
                # binarize
                LGN_julich_mask_img[LGN_julich_mask_img >= 0.3] = 1
                LGN_julich_mask_img[LGN_julich_mask_img < 0.3] = 0
                ...
                img1_data = LGN_julich_mask_img.numpy()
                img2_data = PUi_julich_mask_img.numpy()
                img3_data = PUm_julich_mask_img.numpy()
                img4_data = PUl_julich_mask_img.numpy()

                union_img = (img1_data > 0) | (img2_data > 0) | (img3_data > 0) | (img4_data > 0)
        ...
        for hemi in ['lh', 'rh']:
            ...
            prob_PT_roi = 'Area-Te-2.1_'+hemi+'_MNI152.nii.gz'
            prob_STS1_roi = 'Area-STS1_'+hemi+'_MNI152.nii.gz'
            ...
            if os.path.exists(transformed_mask_path):
                ...
                print("File does not exist. Creating it now")
                # load julich masks
                PT_julich_mask = op.join(paths_julich, prob_PT_roi)
                PT_julich_mask_img = ants.image_read(PT_julich_mask)

                STS1_julich_mask = op.join(paths_julich, prob_STS1_roi)
                STS1_julich_mask_img = ants.image_read(STS1_julich_mask)


                # binarize
                PT_julich_mask_img[PT_julich_mask_img >= 0.3] = 1
                PT_julich_mask_img[PT_julich_mask_img < 0.3] = 0

                STS1_julich_mask_img[STS1_julich_mask_img >= 0.3] = 1
                STS1_julich_mask_img[STS1_julich_mask_img < 0.3] = 0

                img1_data = PT_julich_mask_img.numpy()
                img2_data = STS1_julich_mask_img.numpy()

                union_img = (img1_data > 0) | (img2_data > 0)
                union_img = union_img.astype(PT_julich_mask_img.dtype)  # Keep same data type
                
                # Convert back to ANTs image
                julich_mask_img = ants.from_numpy(union_img, origin=PT_julich_mask_img.origin, spacing=PT_julich_mask_img.spacing, direction=PT_julich_mask_img.direction)
