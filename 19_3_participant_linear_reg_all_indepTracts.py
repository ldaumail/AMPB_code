...
tract_order = ['PTR', 'STR', 'InferiorLongitudinal', 
...
rsquared = np.full((n_subj, n_tracts, len(hemis)), np.nan) #goodness of fit
reliability = np.full((n_subj, n_tracts, len(hemis)), np.nan)
rnd_run_idx = np.full((n_subj, 3, len(hemis)), np.nan)
trained_coefs = np.zeros((n_subj, n_tracts, len(hemis)))  # scalar summary per tract/run
delta_mse = np.full((n_subj, n_tracts, len(hemis)), np.nan) #
...
for h, hemi in enumerate(hemis):
    ...
    predicted = np.full((n_subj, n_tracts, n_masked), np.nan)  # predicted maps per run
    ...
    for s, participant in enumerate(participants):
        ...
        for tract_idx in range(n_tracts):
            ...
            X = np.squeeze(norm_density_data[hemi][s][tract_idx,]).reshape(-1, 1)
            y = np.squeeze(C_mean[s, :])
            ...
            trained_coefs[s, tract_idx, h] = linreg.coef_.copy()
            # linreg.intercept_

            # Predict (optional)
            y_pred = linreg.predict(X)
            predicted[s, tract_idx, :] = y_pred
        

            # Reliability (if needed)
            reliability[s, tract_idx, h] = vertex_bootstrap_reliability(all_C[s,:,:])
...
for h, hemi in enumerate(hemis):
    for g, group in enumerate(groups):
        ...
        # ----------------------------
        img_out_dir = op.join(bids_path, "analysis", "diff2func_model_fits", "wb_participants_linearreg",  "surface_pngs", "mean_predicted_func")
        os.makedirs(img_out_dir, exist_ok=True)
...
for h, hemi in enumerate(hemis):
    ...
    for group in groups:
        ...
        if len(group_idx) == 0:
            continue  # skip if no participants in this group

        for t, tract in enumerate(tract_order):

            # collect data: shape (n_group_subj, n_vertices_roi)
            data = np.array([
                norm_density_data[hemi][s][t]
                for s in group_idx
            ])
            ...
            # ----------------------------
            mean_map = np.nanmean(data, axis=0)

            # ----------------------------
            # build full surface map
            # ----------------------------
            avg_surf_map = np.full((n_vertices,), np.nan, dtype=np.float32)
            avg_surf_map[wang_hmt_vertices] = mean_map
            ...
            out_png = op.join(
                img_out_dir,
                f"avg_hemi-{hemi}_desc-{tract}_group-{group}_inflated.png"
            )
...
for h, hemi in enumerate(hemis):
    ...
    for g, group in enumerate(groups):
        ...
        # Functional Contrast Map visualization
        # ----------------------------
        img_out_dir = op.join(bids_path, "analysis", "diff2func_model_fits", "wb_participants_linearreg",  "surface_pngs", "mean")
        os.makedirs(img_out_dir, exist_ok=True)
...
for ax, hemi in zip(axes, hemi_labels):
    ...
    # 1. Bar Plot 
    # 'dodge=True' is default, but we keep hue here
    sns.barplot(
        data=df_h, x="Tract", y="Beta", hue="Group",
        palette=palette, ax=ax, capsize=0.1, errorbar='se',
        alpha=0.6, edgecolor="black", linewidth=2, 
        hue_order=["EB", "NS"] # Ensure consistent ordering
    )

    # 2. Stripplot (The Overlap Fix)
    # MUST include 'hue' and 'dodge=True' to align with the bars
    sns.stripplot(
        data=df_h, x="Tract", y="Beta", hue="Group",
        palette=palette, ax=ax, size=7, jitter=0.15, 
        alpha=0.9, edgecolor="black", linewidth=1,
        dodge=True, hue_order=["EB", "NS"], # Align dots to shifted bars
        legend=False # Prevent duplicate legends
    )

    # 3. MANUAL LEGEND (Proxy objects)
    ...
    ax.tick_params(axis='both', which='major', labelsize=16, width=2.5)

    # Secondary reinforcement of bold labels
...
for ax, hemi in zip(axes, hemi_labels):
    ...
    for _, row in sem_h.iterrows():
        ...
        x_loc = tract_order.index(row["Tract"]) + group_offset[row["Group"]]
    ...
    for _, row in nc_h.iterrows():
        ...
        x_center = tract_order.index(row["Tract"]) + group_offset[row["Group"]]
