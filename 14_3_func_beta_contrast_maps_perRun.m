...
for n =1:length(participants)
    ...
    for i = 1:length(conds) % for each task
        ...
        for f =1:length(currBold)
            [data,t] = create_data_matrix(currBold(f), TR);
            [design,stimdur,condnames] = create_design_matrix(currEvents(f), t);
            ...
            switch conds(i).task
                case {'mtlocal', 'ptlocal'} % collapse across runs and perform constrast
                    ...
                    modelmd = bsxfun(@times, modelmd(:,(f-1)*matSize(end)/length(currBold)+1:f*matSize(end)/length(currBold)), results.meanvol); % amplitude %@times = elementwise multiplication
                    modelmd = reshape(modelmd, nv, matSize(end)/length(currBold)); % flatten
                    ...
                    e = q .* var(dataMat{:}, [], 2, 'omitnan'); % variance of residuals for each vertex
                    ...
                    switch glmMode
                        case 'surface'
                            saveName = sprintf( ...
                                '%s_task-%s_run-%d_%s_space-fsnative_desc-%s_tstat.mgz', ...
                                participant, conds(i).task,f, conds(i).hemi, contrastDesc);
                        case 'volume'
                            saveName = sprintf( ...
                                '%s_task-%s_run-%d_space-T1w_desc-%s_tstat.nii.gz', ...
                                participant, conds(i).task, f, contrastDesc);
                    end
