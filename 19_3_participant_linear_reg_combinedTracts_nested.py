...
tract_order = ['CallosumPosteriorParietal', 'CallosumOccipital', 'LeftVerticalOccipital', 'LeftInferiorFrontooccipital', 'LeftInferiorLongitudinal', 'LeftSuperiorLongitudinal', 'LeftPosteriorArcuate',  'LeftArcuate'] 
...
for h, hemi in enumerate(hemis):
    ...
    vmin, vmax = -5.0, 5.0
    for t, tract in enumerate(tract_order):
        surf_map = np.full((n_vertices, len(participants)), np.nan, dtype=np.float32)
        ...
        for s, participant in enumerate(participants):
            ...
            surf_map[wang_hmt_vertices, s] = norm_density_data[hemi][s][t]#predicted_maps[hemi][s, :]
        avg_surf_map = np.mean(surf_map, axis = 1)

        img_out_dir = op.join(bids_path, "analysis", "diff2func_model_fits", "wb_participants_linearreg", "surface_pngs", "avg")
        os.makedirs(img_out_dir, exist_ok=True)
        ...
        out_png = op.join(
            img_out_dir,
            f"avg_hemi-{hemi}_desc-{tract}_inflated.png"
        )
...
saveDir = op.join(bids_path, "analysis", "plots")
os.makedirs(saveDir, exist_ok=True)
plt.savefig(op.join(saveDir, "beta_barplot_linreg_participants_combined_tracts.png"), dpi=300, bbox_inches='tight')
plt.show()
...
saveDir = op.join(bids_path, "analysis", "plots")
os.makedirs(saveDir, exist_ok=True)
plt.savefig(op.join(saveDir, "pearson_barplot_linreg_participants_combined_tracts.png"), dpi=300, bbox_inches='tight')
plt.show()