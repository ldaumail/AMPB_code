...
tract_order = ['MTxLGN','MTxPT', 'MTxSTS1', 'MTxPU'] #'MTxPT','MTxPU', 'MTxFEF','MTxhIP', 'MTxV1'
...
trained_coefs = np.full((n_subj, n_tracts,len(hemis)), np.nan)         # coef[test_subject, tract]
hemis = ["L", "R"]
for h, hemi in enumerate(hemis):
    ...
    for test_idx in range(n_subj):
        ...
        trained_coefs[test_idx,:, h] = linreg.coef_.copy()
    ...
    #     outpath = os.path.join(out_dir, participant, f"{participant}_hemi-{hemi}_label-combined_desc-predicted_contrast.mgz")
...
plt.title("Prediction Accuracy (LGN,PT,STS1, PU, FEF, and hIP Tracts Combined)\n Grouped by Hemisphere and Group")
...
plt.savefig(
    op.join(saveDir, "pearsonrs_ridgecv_combtracts-LGN-PT-STS1-PU-FEF-hIP_grouped_barplot_wang.png"),
...
plt.savefig(
    op.join(saveDir, "pearsonrs_linearcv_loso_combtracts_jitter_wang_LGN-PT-PU-STS1.png"),
    dpi=300, bbox_inches="tight"
)
plt.show()
...
plt.savefig(op.join(saveDir, "betas_linearcv_loso_combinedTracts_jitter_wang_STD.png"),
            dpi=300, bbox_inches='tight')
plt.show()
